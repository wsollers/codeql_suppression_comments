 ...
 class SuppressionComment extends Comment {
   ...
   SuppressionComment() {
     ...
     (
      ...
      // Exclude comments that contain "codeql" (case-insensitive)
      not text.regexpMatch("(?i).*\\bcodeql\\b.*"
     )
   }
   ...
   predicate covers(string filepath, int startline, int startcolumn, int endline, int endcolumn) {
    exists(int commentLine |
      ...
      (
        ...
        startcolumn = 1 and
        endcolumn = 200  // Arbitrary large column number to cover the whole line
      )
    )
  }
